...
function FlightCard({flightData, setBookFlightData }) {
    ...
    const { flightNo, from, to, category, date, departureTime, duration, arrivalTime, totalPrice, aircraft, airline, stops } = flightData;
    ...
    async function book_flight(event) {
        ...
        try {
            setIsLoading(true)
            const response = await fetch(backendUrl+'/api/canbook', {
                method: 'POST',
                headers: {
